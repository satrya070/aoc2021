...
def runner(proc_str: str):
    ...
    while running:
        ...
        if len(stack) != 0 and stack[-1]["processed"] == stack[-1]["count"]:
            ...
            node_value = node_processor(done_node['type_version'], done_node['subpack_values'])

            # stop if stack has reached zero, no more parents to update holmes
            if len(stack) == 0:
                break

            # TODO update/add to parent subpack_values
            stack[-1]["subpack_values"].append(node_value)
